angular
	.module('app')
	.controller('seckillController', seckillController);

seckillController.$inject = ['$scope', '$api'];

function seckillController($scope, $api) {
	var events = {};
  $scope.activityStatus ="3";
  $api.get("activity/activityType/1").then(function(resp){
    if(resp.data.code ==200){
      ...
      $scope.seckilllist =resp.data.activityList;
    }else{
  ...
  events.activityStatus = function (){
    $api.get("activity/list",{activityType:1,activityStatus:$scope.activityStatus}).then(function(resp){
      if(resp.data.code ==200){
        $.each(resp.data.activityList,function(index,item){
          if(item.activityStatus ==0){
            item.status ="未开始" ;
  ...
  events.seckillsear = function(){
    ...
    if($scope.activityName){
      $api.get("activity/list",{activityType:1,activityName:$scope.activityName}).then(function(resp){
        if(resp.data.code ==200){
          $.each(resp.data.activityList,function(index,item){
            if(item.activityStatus ==0){
              item.status ="未开始" ;
