package com.tyiti.easycommerce.util.report.util;
...
import com.tyiti.easycommerce.util.ExcelUtil;
import com.tyiti.easycommerce.util.XMLHandler;
import com.tyiti.easycommerce.util.XmlTools;
import com.tyiti.easycommerce.util.report.model.ExcelReportColumnModel;
import com.tyiti.easycommerce.util.report.model.ExcelReportModel;
...
public class ExcelReportUtil {
	...
	public static HSSFWorkbook genExcelReport(String templateFilePath,
			List<? extends Object> datas) throws Exception {
		File templateFile = ResourceUtils.getFile("classpath:"+Constants.EXPORT_TEMP_PATH+templateFilePath);
		...
		for (int index = 0; index < columns.size(); index++) {
			...
			if((!StringUtils.isEmpty(dataType))&&dataType.equalsIgnoreCase("sequence")){
				...
				if(value!=null){
				    ...
				    int len = String.valueOf(value).getBytes().length*256 ;
				    if (len <3000){
				    	len = 3000 ; 
				    } 
				    row.getSheet().setColumnWidth(index,len );
				}else{
					  row.getSheet().setColumnWidth(index,3000 );
				}
